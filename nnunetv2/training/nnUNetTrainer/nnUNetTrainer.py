...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        for k in inspect.signature(self.__init__).parameters.keys():
            # 支持子類添加額外參數：如果參數在父類的 locals() 中不存在，則跳過
            # 子類應該在 super().__init__() 之後手動添加這些參數到 my_init_kwargs
            if k in locals():
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file('Val_Pseudo dice', [np.round(i, decimals=4) for i in
        ...
        if (current_epoch + 1) % self.save_every == 0 and current_epoch != (self.num_epochs - 1):
            ...
            self.save_checkpoint(join(self.output_folder, f'checkpoint_latest.pth'))
