...
class GatedCrossAttentionFusion(nn.Module):
    ...
    支援三種模式：
    1. 'none' / False:  baseline，無門控
    2. 'headwise':  每個 head 一個門控標量（參數少，效率高）
    3. 'elementwise': 每個元素一個門控值（參數多，表達能力強）
    
    論文來源:  NeurIPS 2025 Oral
    "Gated Attention for Large Language Models:  Non-linearity, Sparsity, and Attention-Sink-Free"
    """
    def __init__(self, channels:  int, cli_dim: int = 320, num_heads: int = 8, 
                 dropout: float = 0.1, gating_mode: str = 'headwise'):
        """
        Args:
            channels:  影像特徵的通道數
            cli_dim: 臨床特徵的維度
            ...
            dropout: dropout 比率
            gating_mode: 門控模式
                - 'none' 或 False: 不使用門控 (baseline)
                - 'headwise': 頭級門控 (每個head一個標量)
                - 'elementwise': 元素級門控 (每個元素一個標量)
        """
        super().__init__()
        self.channels = channels
        self. num_heads = num_heads
        self.head_dim = channels // num_heads
        self.gating_mode = gating_mode if gating_mode else 'none'
        
        assert channels % num_heads == 0, f"channels ({channels}) 必須能被 num_heads ({num_heads}) 整除"
        assert self.gating_mode in ['none', 'headwise', 'elementwise'], \
            f"gating_mode 必須是 'none', 'headwise', 或 'elementwise'，得到:  {self.gating_mode}"
        
        # ========== Query 投影層（根據門控模式調整輸出維度）==========
        if self.gating_mode == 'none': 
            # Baseline: 只輸出 query
            self.img_query = nn.Conv3d(channels, channels, 1)
            
        elif self.gating_mode == 'headwise':
            # Headwise: 輸出 query + 每個head的門控標量
            # 輸出維度: channels (query) + num_heads (gate scores)
            self.img_query = nn.Conv3d(channels, channels + num_heads, 1)
            print(f"  [Headwise Gating] 額外參數: {num_heads} 個門控標量/head")
            
        elif self.gating_mode == 'elementwise':
            # Elementwise: 輸出 query + 每個元素的門控值
            # 輸出維度: channels (query) + channels (gate scores)
            self.img_query = nn.Conv3d(channels, channels * 2, 1)
            print(f"  [Elementwise Gating] 額外參數: {channels} 個門控值/元素")
        
        # 臨床特徵投影 (作為 Key 和 Value)
        self.num_clinical_tokens = 4
    ...
    def forward(self, img_feat, cli_feat):
        ...
        # ========== 步驟 2: Key & Value 投影 ==========
        key_raw = self.cli_key(cli_feat)
        value_raw = self.cli_value(cli_feat)
        ...
        # ========== 步驟 7: 融合與殘差連接 ==========
        fused = torch.cat([img_feat, attn_output], dim=1)
